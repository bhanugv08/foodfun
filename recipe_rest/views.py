...
class RecipeViewset(viewsets.ViewSet):
    ...
    def get_recipe_by_name(self, request):
        recipes = list(Recipe.objects.get(recipe_name=request.data.get("name")).values("id", "recipe_name", "ingredients"))
        return Response(recipes)

    @action(methods=["GET"], detail=False)
    def get_recipe_by_id(self, request):
        recipes = list(Recipe.objects.get(id=request.data.get("id")).values("id", "recipe_name", "ingredients"))
        return Response(recipes)
